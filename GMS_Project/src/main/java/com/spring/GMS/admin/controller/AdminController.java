...
public class AdminController {
	...
	@RequestMapping(value="/adminartist" , method=RequestMethod.GET)
	public ModelAndView adminartist(@RequestParam(name = "onePageViewCount"  , defaultValue = "10")    int onePageViewCount,
									  @RequestParam(name = "currentPageNumber" , defaultValue = "1")     int currentPageNumber,
									  @RequestParam(name = "searchKeyword"     , defaultValue = "total") String searchKeyword,
									  @RequestParam(name = "searchWord"        , defaultValue = "")      String searchWord) throws Exception {
		...
		ModelAndView mv = new ModelAndView();
		mv.setViewName("admin/adminartist");
		...
		searchInfo.put("searchKeyword"    , searchKeyword);
		searchInfo.put("searchWord"       , searchWord);
		List<ArtistDto> artistList = adminService.allartistList(searchInfo);
		...
		// 전체페이지 개수 = 전체게시글 수 / 한페이지에서 보여지는 글수
		int totalBoardCount = adminService.getAllartistCount(searchCountInfo);
		int addPage = totalBoardCount % onePageViewCount == 0 ? 0 : 1; 		// 나머지가 0이면 추가 x , 나머지가 0이 아니면 +1 페이지 처리
		int totalPageCount = totalBoardCount / onePageViewCount + addPage;
	...
	@RequestMapping(value="/artistadd" , method=RequestMethod.GET)
	public ModelAndView artistadd() throws Exception  {
		return new ModelAndView("admin/artistadd");
	}
	
	@RequestMapping(value="/artistadd" , method=RequestMethod.POST)
	public ResponseEntity<String> artistadd(MultipartHttpServletRequest multipartRequest , HttpServletResponse response) throws Exception {
		...
		artistDto.setArtist(multipartRequest.getParameter("artist"));
		artistDto.setArtistComent(multipartRequest.getParameter("artistComent"));
		artistDto.setMainArtist(multipartRequest.getParameter("mainArtist"));
		...
		if (file.hasNext()) {
			...
			if (!uploadFile.getOriginalFilename().isEmpty()) {
				...
				artistDto.setArtistImage(fileName);
			}
		}
			
		adminService.artistadd(artistDto);
		
		HttpHeaders responseHeaders = new HttpHeaders();
		responseHeaders.add("Content-Type", "text/html; charset=utf-8");
		String message= "<script>";
			   message += " alert('작가 등록 성공');";
			   message +=" location.href='" + multipartRequest.getContextPath() + "/admin/adminartist';";
			   message +="</script>";
	...
	public ModelAndView adminorder(@RequestParam(name = "onePageViewCount"  , defaultValue = "10")    int onePageViewCount,
									  @RequestParam(name = "currentPageNumber" , defaultValue = "1")     int currentPageNumber,
									  @RequestParam(name = "searchKeyword"     , defaultValue = "total") String searchKeyword,
									  @RequestParam(name = "searchWord"        , defaultValue = "")      String searchWord) throws Exception {
	...
	public ResponseEntity<Object> modifyDeliveryState(@RequestParam Map<String, String> deliveryMap) throws Exception {
		adminService.modifyDeliveryState(deliveryMap);
